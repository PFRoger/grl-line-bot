...
4. LIFF 會員中心：查看等級、點數、優惠券、進行中訂單進度
5. Rich Menu：查詢紀錄、開始購物、購物車、購物指南、穿搭靈感、會員中心
...
| `buildFlexMessage()` | 建立商品報價 Flex Message |
...
| `processOrderReturn()` | 退單：扣回點數、回扣年度消費、重算等級、通知買家 |
| `getMember()` | 讀取會員資料（找不到回傳 null） |
| `buildCartHtml()` | 產生 LIFF 購物車 HTML |
| `handlePostback()` | 處理所有 postback 事件 |
