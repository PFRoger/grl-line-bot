...
`index.js`（~3,927 行）保留所有路由和 Express 邏輯；核心運算已拆至 `lib/`：

| 模組 | 說明 | 主要 export |
|------|------|-------------|
| `lib/config.js` | 環境變數、常數、LINE/Sheets 客戶端單例 | `ADMIN_KEY`, `SHEET_ID`, 所有 sheet 名稱, `lineClient`, `getSheetsClient`, `line` |
| `lib/colors.js` | GRL 日文顏色 → 中文對照 | `COLOR_MAP_OBJ`, `COLOR_KEYS`, `hasColorKeyword`, `translateColorWithJp` |
| `lib/pricing.js` | 報價計算、匯率抓取 | `calcSuggestedPrice`, `fetchRate`, `fmtJPY`, 運費/費率常數 |
| `lib/weight.js` | 商品重量估算（品類 + 素材 + 袖長） | `estimateWeight` |
| `lib/grl.js` | 爬 GRL 商品頁、解析庫存 | `scrapeGRL`, `extractProductId`, `parseStockFromHtml`, `calcQStatus`, `translateArrival` |
| `lib/sheets.js` | Google Sheets 讀寫（購物車、訂單、查詢紀錄） | `getCartItems`, `addToCartSheet`, `submitOrder`, `logQueryToSheet`, `getUserQueryHistory`, `checkAndSetBotReply` 等 17 個 |
| `lib/flex.js` | 組 GRL 報價/歷史/教學 Flex Message | `buildFlexMessage`, `buildAddToCartFlex`, `buildHistoryFlexMessage`, `buildTutorialFlexMessage`, `buildWelcomeFlexMessage` |
| `lib/zozo.js` | ZOZO 報價 Flex Message、顏色/尺寸對照 | `buildZOZOFlexMessage`, `zozoColorLabel`, `zozoSizeName`, `ZOZO_COLOR_MAP` |
| `lib/member.js` | 會員、點數、優惠券、邀請制度（金流相關） | `getMember`, `createMember`, `deductMemberPoints`, `markCouponUsed`, `processOrderCompletion`, `processOrderReturn`, `issueCoupons`, `bindReferralCode` 等 23 個 |
...
```
.
├── index.js              # 主程式（Express + LINE Bot + LIFF + 管理後台，~3927 行）
├── lib/
│   ├── config.js         # 環境變數、常數、LINE/Sheets 客戶端
│   ├── colors.js         # GRL 顏色對照表
│   ├── pricing.js        # 報價計算、匯率
│   ├── weight.js         # 重量估算
│   ├── grl.js            # GRL 爬蟲
│   ├── sheets.js         # Google Sheets 操作（購物車、訂單）
│   ├── flex.js           # GRL Flex Message 組裝
│   ├── zozo.js           # ZOZO Flex Message 組裝
│   └── member.js         # 會員、點數、優惠券、邀請
├── chrome-extension/     # ZOZO 查詢代理 Chrome Extension
├── gas_program_V7.6.txt  # GRL 庫存監控 GAS（獨立部署）
├── assets/               # Rich Menu 卡片圖片
├── package.json
